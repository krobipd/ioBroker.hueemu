    ...
    public async setLightState(lightId: string, stateUpdate: LightStateUpdate): Promise<LightStateResult[]> {
        ...
        for (const [ key, value ] of Object.entries(stateUpdate)) {
            ...
            try {
                ...
                await this.adapter.setForeignStateAsync(stateId, { val: convertedValue, ack: false });
    ...
    private convertValueForState(stateName: string, value: unknown): ioBroker.StateValue {
        switch (stateName) {
            ...
            case 'bri':
                ...
                return typeof value === 'number' ? Math.min(254, Math.max(1, value)) : 254;
            case 'hue':
                return typeof value === 'number' ? Math.min(65535, Math.max(0, value)) : 0;
            case 'sat':
                return typeof value === 'number' ? Math.min(254, Math.max(0, value)) : 254;
            case 'ct':
                return typeof value === 'number' ? Math.min(500, Math.max(153, value)) : 250;
            case 'xy':
