...
export interface LightServiceAdapter {
    ...
    getStatesOf(parentDevice: string | undefined, parentChannel: string | undefined, callback: ioBroker.GetObjectsCallback3<ioBroker.StateObject>): void;
    getState(id: string, callback: ioBroker.GetStateCallback): void;
    setState(id: string, state: ioBroker.SettableState, callback?: ioBroker.SetStateCallback): void;
}
...
export class LightService {
    ...
    public async getLightById(lightId: string): Promise<Light> {
        ...
        const light: Light = {
            ...
            uniqueid: (additionalData.uniqueid as string) || `00:17:88:01:00:00:00:${lightId.padStart(2, '0')}-0b`,
            ...additionalData
        };
    ...
    public async setLightState(lightId: string, stateUpdate: LightStateUpdate): Promise<LightStateResult[]> {
        ...
        const stateKeys = new Set(stateObjects.map(s =>
     ...
     */
    private getStatesOf(device: string, channel: string): Promise<ioBroker.StateObject[]> {
        return new Promise((resolve, reject) => {
