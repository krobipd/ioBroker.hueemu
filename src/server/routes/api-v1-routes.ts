...
export async function apiV1Routes(
    ...
    fastify.put<{ Params: LightParams }>('/api/:username/lights/:id/state', async (request, reply) => {
        await handleErrors(request, reply, async () => {
            ...
            const isAuth = await handler.isUserAuthenticated(username);
            if (!isAuth && !handler.isAuthDisabled()) {
                throw HueApiError.unauthorizedUser(`/api/${username}/lights/${id}/state`);
            }
    ...
    fastify.get<{ Params: UsernameParams }>('/api/:username/groups', async (request, reply) => {
        await handleErrors(request, reply, async () => {
            const { username } = request.params;
    ...
    fastify.get<{ Params: UsernameParams }>('/api/:username/schedules', async (request, reply) => {
        await handleErrors(request, reply, async () => {
            const { username } = request.params;
    ...
    fastify.get<{ Params: UsernameParams }>('/api/:username/scenes', async (request, reply) => {
        await handleErrors(request, reply, async () => {
            const { username } = request.params;
    ...
    fastify.get<{ Params: UsernameParams }>('/api/:username/sensors', async (request, reply) => {
        await handleErrors(request, reply, async () => {
            const { username } = request.params;
    ...
    fastify.get<{ Params: UsernameParams }>('/api/:username/rules', async (request, reply) => {
        await handleErrors(request, reply, async () => {
            const { username } = request.params;
    ...
    fastify.get<{ Params: UsernameParams }>('/api/:username/resourcelinks', async (request, reply) => {
        await handleErrors(request, reply, async () => {
            const { username } = request.params;
