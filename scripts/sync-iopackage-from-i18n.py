...
i18n-states.ts ist single-source-of-truth, die common.name/desc-Blocks in
io-package.json werden bei jedem Release neu aus i18n-states.ts gerendert.
...
NAME_MAPPING = {
    "startPairing": "startPairingName",
    "disableAuth": "disableAuthName",
    "clients": "clientsFolder",
}

# Mapping: io-package.json instanceObjects._id  →  i18n-states.ts STATE_NAMES-Key (desc)
# hueemu keeps desc translations in STATE_NAMES (no separate STATE_DESCS)
DESC_MAPPING = {
    "startPairing": "startPairingDesc",
    "disableAuth": "disableAuthDesc",
}
...
def main() -> int:
    ...
    # hueemu: desc keys live in STATE_NAMES (no separate STATE_DESCS)
    name_changes, name_errors = sync_field(instance_objects, state_names, NAME_MAPPING, "name")
    desc_changes, desc_errors = sync_field(instance_objects, state_names, DESC_MAPPING, "desc")

    errors = name_errors + desc_errors
    total_changes = name_changes + desc_changes
